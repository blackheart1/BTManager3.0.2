...
** File include/message_parser.php 2018-09-22 00:00:00 Thor
**
** CHANGES
**
...
class bbcode_firstpass extends bbcode
{
        ...
        foreach ($this->bbcodes as $bbcode_name => $bbcode_data)
        {
            if (isset($bbcode_data['disabled']) && $bbcode_data['disabled'])
            {
                foreach ($bbcode_data['regexp'] as $regexp => $replacement)
                {
                    ...
                    {
                        $this->warn_msg[] = sprintf($user->lang['_UNAUTHORISED_BBCODE'] , '[' . $bbcode_name . ']');
                        continue;
                    }
                }
            }
    ...
    */
    function bbcode_init()
    {
        ...
        {
            $firstrip = $row['first_pass_replace'];
            $this->bbcodes[$row['bbcode_tag']] = array(
                'bbcode_id' => (int) $row['bbcode_id'],
                'regexp'    => array($row['first_pass_match'] => str_replace('$uid', $this->bbcode_uid, $firstrip))
            );
        }
    ...
    */
    function bbcode_size($stx, $in)
    {
        ...
        {
            $this->warn_msg[] = sprintf($user->lang['_MAX_FONT_SIZE_EXCEEDED'], $config['max_' . $this->mode . '_font_size']);
    ...
    */
    function bbcode_img($in)
    {
        ...
        if ($config['max_' . $this->mode . '_img_height'] || $config['max_' . $this->mode . '_img_width'])
        {
            ...
            {
                $error = true;
                $this->warn_msg[] = $user->lang['_UNABLE_GET_IMAGE_SIZE'];
            }
            else
            {
                ...
                {
                    $error = true;
                    $this->warn_msg[] = sprintf($user->lang['_MAX_IMG_HEIGHT_EXCEEDED'], $config['max_' . $this->mode . '_img_height']);
                }

                if ($config['max_' . $this->mode . '_img_width'] && $config['max_' . $this->mode . '_img_width'] < $stats[0])
                {
                    $error = true;
                    $this->warn_msg[] = sprintf($user->lang['_MAX_IMG_WIDTH_EXCEEDED'], $config['max_' . $this->mode . '_img_width']);
                }
            }
        }
    ...
    */
    function bbcode_flash($width, $height, $in)
    {
        ...
        if ($config['max_' . $this->mode . '_img_height'] || $config['max_' . $this->mode . '_img_width'])
        {
            ...
            {
                $error = true;
                $this->warn_msg[] = sprintf($user->lang['_MAX_FLASH_HEIGHT_EXCEEDED'], $config['max_' . $this->mode . '_img_height']);
            }

            if ($config['max_' . $this->mode . '_img_width'] && $config['max_' . $this->mode . '_img_width'] < $width)
            {
                $error = true;
                $this->warn_msg[] = sprintf($user->lang['_MAX_FLASH_WIDTH_EXCEEDED'], $config['max_' . $this->mode . '_img_width']);
            }
        }
            ...
            if ($tok == ']')
            {
                ...
                else if (preg_match('#^quote(?:=&quot;(.*?)&quot;)?$#is', $buffer, $m) && substr($out, -1, 1) == '[')
                {
                    ...
                    if ($config['max_quote_depth'] && sizeof($close_tags) >= $config['max_quote_depth'])
                    {
                        ...
                        $error_ary['quote_depth'] = sprintf($user->lang['_QUOTE_DEPTH_EXCEEDED'], $config['max_quote_depth']);
    ...
    */
    function validate_url($var1, $var2)
    {
        ...
        if ($valid)
        {
            ...
            if (strpos($url, $siteurl) !== false && strpos($url, 'sid=') !== false)
            {
...
class parse_message extends bbcode_firstpass
{
    ...
    */
    ...
    {
        // Init BBCode UID
        $this->bbcode_uid = substr(base_convert(substr(md5(microtime()), 4, 16), 16, 36), 0, 8);
    ...
    */
    ...
    {
        global $config, $db, $db_prefix, $user, $siteurl;
        ...
        $message_length = ($mode == 'post') ? strlen($this->message) : strlen(preg_replace('#\[\/?[a-z\*\+\-]+(=[\S]+)?\]#ius', ' ', $this->message));

        // Maximum message length check. 0 disables this check completely.
        if ((int) $config['max_' . $mode . '_chars'] > 0 && $message_length > (int) $config['max_' . $mode . '_chars'])
        {
            $this->warn_msg[] = sprintf($user->lang['_TOO_MANY_CHARS_' . strtoupper($mode)], $message_length, (int) $config['max_' . $mode . '_chars']);
        ...
        if ($mode === 'post')
        {
            ...
            {
                $this->warn_msg[] = (!$message_length) ? $user->lang['_TOO_FEW_CHARS'] : sprintf($user->lang['_TOO_FEW_CHARS_LIMIT'], $message_length, (int) $config['min_post_chars']);
                return (!$update_this_message) ? $return_message : $this->warn_msg;
            }
        }
        ...
        if ($mode === 'post' && $this->message === '')
        {
            $this->warn_msg[] = $user->lang['_TOO_FEW_CHARS'];
            return (!$update_this_message) ? $return_message : $this->warn_msg;
        }
        ...
        {
            $this->warn_msg[] = sprintf($user->lang['_TOO_MANY_URLS'], $config['max_' . $mode . '_urls']);
            return (!$update_this_message) ? $return_message : $this->warn_msg;
        }
    ...
    */
    function format_display($allow_bbcode, $allow_magic_url, $allow_smilies, $update_this_message = true)
    {
        ...
        $this->message = parse_smiles($this->message);
    ...
    function smilies($max_smilies = 0)
    {
        ...
        if (!is_array($match))
        {
            ...
            while ($row = $db->sql_fetchrow($result))
            {
                ...
                $replace[] = '<!-- s' . $row['code'] . ' --><img src="smiles/' . $row['file'] . '" alt="' . $row['alt'] . '" title="' . $row['alt'] . '" /><!-- s' . $row['code'] . ' -->';
            }
            $db->sql_freeresult($result);
        }
        ...
        if (sizeof($match))
        {
            if ($max_smilies)
            {
                ...
                {
                    $this->warn_msg[] = sprintf($user->lang['_TOO_MANY_SMILIES'], $max_smilies);
                    return;
                }
            }
    ...
    */
    function parse_attachments($form_name, $mode, $forum_id, $submit, $preview, $refresh, $is_message = false)
    {
        ...
        if ($submit && in_array($mode, array('post', 'reply', 'quote', 'edit')) && $upload_file)
        {
            ...
            {
                $filedata = add_attach($form_name, $forum_id, false, '', $is_message);
                ...
                if ($filedata['post_attach'] && !sizeof($error))
                {
                    ...
                    $db->sql_query('INSERT INTO ' . $db_prefix . '_attachments ' . $db->sql_build_array('INSERT', $sql_ary));
            ...
            {
                $error[] = sprintf($user->lang['_TOO_MANY_ATTACHMENTS'], $cfg['max_attachments']);
            }
        }
            ...
            if ($delete_file)
            {
                ...
                if ($index !== false && !empty($this->attachment_data[$index]))
                {
                    ...
                    if ($this->attachment_data[$index]['is_orphan'])
                    {
                        ...
                        if ($row)
                        {
                            attach_unlink($row['physical_filename'], 'file');

                            if ($row['thumbnail'])
                            {
                                attach_unlink($row['physical_filename'], 'thumbnail');
                            }

                            $db->sql_query('DELETE FROM ' . $db_prefix . '_attachments WHERE attach_id = ' . (int) $this->attachment_data[$index]['attach_id']);
                        }
                    }
                    else
                    {
    ...
    */
    ...
    {
        global $user, $db, $db_prefix, $phpbb_root_path, $phpEx, $config;

        $this->filename_data['filecomment'] = request_var('filecomment', '', true);
        $attachment_data = (isset($_POST['attachment_data'])) ? $_POST['attachment_data'] : array();
        $this->attachment_data = array();

        $check_user_id = ($check_user_id === false) ? $user->id : $check_user_id;
        ...
        if (sizeof($not_orphan))
        {
            pmbt_trigger_error('NO_ACCESS_ATTACHMENT1', E_USER_ERROR);
        }
        ...
        if (sizeof($orphan))
        {
            pmbt_trigger_error('NO_ACCESS_ATTACHMENT2', E_USER_ERROR);
        }
    ...
    */
    function parse_poll(&$poll)
    {
        ...
        {
            $this->warn_msg[] = $user->lang['_NO_POLL_TITLE'];
        }
        else
        {
            if (utf8_strlen(preg_replace('#\[\/?[a-z\*\+\-]+(=[\S]+)?\]#ius', ' ', $this->message)) > 100)
            {
                $this->warn_msg[] = $user->lang['_POLL_TITLE_TOO_LONG'];
            }
            ...
            {
                $this->warn_msg[] = $user->lang['_POLL_TITLE_COMP_TOO_LONG'];
            }
        }
        ...
        {
            $this->warn_msg[] = $user->lang['_TOO_FEW_POLL_OPTIONS'];
        }
        else if ($poll['poll_options_size'] > (int) $config['max_poll_options'])
        {
            $this->warn_msg[] = $user->lang['_TOO_MANY_POLL_OPTIONS'];
        }
        else if ($poll_max_options > $poll['poll_options_size'])
        {
            $this->warn_msg[] = $user->lang['_TOO_MANY_USER_OPTIONS'];
        }
