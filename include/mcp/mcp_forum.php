...
** File mcp/mcp_forum.php 2018-09-22 00:00:00 Thor
**
** CHANGES
**
...
{
    global $template, $db, $user, $auth, $pmbt_cache, $db_prefix, $module;
    global $phpEx, $phpbb_root_path, $config;

    //$user->add_lang(array('viewtopic', 'viewforum'));

    //include_once($phpbb_root_path . 'include/function_display.' . $phpEx);
    ...
    $url = append_sid("{$phpbb_root_path}forum.$phpEx?action_mcp=mcp$url_extra");
    ...
    $template->assign_vars(array(
        ...
        'U_VIEW_FORUM_LOGS'     => ($auth->acl_gets('a_', 'm_', $forum_id)) ? append_sid("{$phpbb_root_path}forum.$phpEx?action_mcp=mcp", 'i=logs&amp;mode=forum_logs&amp;f=' . $forum_id) : '',
    ...
    {
        $read_tracking_join = ' LEFT JOIN ' . $db_prefix . '_topics_track tt ON (tt.topic_id = t.topic_id AND tt.user_id = ' . $user->id . ')';
    ...
    $sql = "SELECT t.*$read_tracking_select
        FROM " . $db_prefix . "_topics t $read_tracking_join
    ...
    foreach ($topic_list as $topic_id)
    {
        ...
        if ($row['topic_status'] == 2)
        {
            $topic_row = array_merge($topic_row, array(
                'U_VIEW_TOPIC'      => append_sid("{$phpbb_root_path}forum.$phpEx?action=viewtopic", "t={$row['topic_moved_id']}"),
                'U_DELETE_TOPIC'    => ($auth->acl_get('m_delete', $forum_id)) ? append_sid("{$phpbb_root_path}forum.$phpEx?action_mcp=mcp", "i=$id&amp;f=$forum_id&amp;topic_id_list[]={$row['topic_id']}&amp;mode=forum_view&amp;action=delete_topic") : '',
                'S_MOVED_TOPIC'     => true,
        ...
        else
        {
            ...
            $topic_row = array_merge($topic_row, array(
                'U_VIEW_TOPIC'      => append_sid("{$phpbb_root_path}forum.$phpEx?action_mcp=mcp", "i=$id&amp;f=$forum_id&amp;t={$row['topic_id']}&amp;mode=topic_view"),

                'S_SELECT_TOPIC'    => ($merge_select && !in_array($row['topic_id'], $source_topic_ids)) ? true : false,
                'U_SELECT_TOPIC'    => $u_select_topic,
                'U_MCP_QUEUE'       => $u_mcp_queue,
                'U_MCP_REPORT'      => ($auth->acl_get('m_report', $forum_id)) ? append_sid("{$phpbb_root_path}forum.$phpEx?action_mcp=mcp", 'i=main&amp;mode=topic_view&amp;t=' . $row['topic_id'] . '&amp;action=reports') : '',
...
{
    global $auth, $db, $db_prefix, $template, $phpEx, $user, $phpbb_root_path;
...
function merge_topics($forum_id, $topic_ids, $to_topic_id)
{
    ...
    if (confirm_box(true))
    {
        ...
        $db->sql_query('UPDATE ' . db_prefix . '_topics_watch SET topic_id = ' . (int) $to_topic_id . ' WHERE ' . $db->sql_in_set('topic_id', $topic_ids));
        $db->sql_return_on_error(false);

        $db->sql_query('DELETE FROM ' . db_prefix . '_topics_watch WHERE ' . $db->sql_in_set('topic_id', $topic_ids));
