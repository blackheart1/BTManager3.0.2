...
** File mcp/mcp_queue.php 2018-09-22 00:00:00 Thor
**
** CHANGES
**
...
class mcp_queue
{
    ...
    {
        global $auth, $db, $db_prefix, $user, $template, $pmbt_cache;
        ...
        switch ($mode)
        {
            case 'approve_details':
                ...
                $post_url = append_sid("{$phpbb_root_path}forum.$phpEx?action=viewtopic", 'f=' . $post_info['forum_id'] . '&amp;p=' . $post_info['post_id'] . '#p' . $post_info['post_id']);
                $topic_url = append_sid("{$phpbb_root_path}forum.$phpEx?action=viewtopic", 'f=' . $post_info['forum_id'] . '&amp;t=' . $post_info['topic_id']);

                $template->assign_vars(array(
                    'S_MCP_QUEUE'           => true,
                    'U_APPROVE_ACTION'      => append_sid("{$phpbb_root_path}forum.$phpEx?action_mcp=mcp", "i=queue&amp;p=$post_id&amp;f=$forum_id"),
                    ...
                    'U_MCP_USER_NOTES'      => append_sid("{$phpbb_root_path}forum.$phpEx?action_mcp=mcp", 'i=notes&amp;mode=user_notes&amp;u=' . $post_info['poster_id']),
                    'U_MCP_WARN_USER'       => ($auth->acl_get('m_warn')) ? append_sid("{$phpbb_root_path}forum.$phpEx?action_mcp=mcp", 'i=warn&amp;mode=warn_user&amp;u=' . $post_info['poster_id']) : '',
                    'U_VIEW_POST'           => $post_url,
                    'U_VIEW_TOPIC'          => $topic_url,

                    'MINI_POST_IMG'         => ($post_unread) ? $user->img('icon_post_target_unread', 'NEW_POST') : $user->img('icon_post_target', 'POST'),

                    'RETURN_QUEUE'          => sprintf($user->lang['RETURN_QUEUE'], '<a href="' . append_sid("{$phpbb_root_path}forum.$phpEx?action_mcp=mcp", 'i=queue' . (($topic_id) ? '&amp;mode=unapproved_topics' : '&amp;mode=unapproved_posts')) . "&amp;start=$start\">", '</a>'),
                    ...
                    'U_LOOKUP_IP'           => ($auth->acl_get('m_info', $post_info['forum_id'])) ? append_sid("{$phpbb_root_path}forum.$phpEx?action_mcp=mcp", 'i=queue&amp;mode=approve_details&amp;f=' . $post_info['forum_id'] . '&amp;p=' . $post_id . '&amp;lookup=' . $post_info['poster_ip']) . '#ip' : '',
                ));
            ...
            case 'unapproved_posts':
                ...
                if ($mode == 'unapproved_posts')
                {
                    $sql = 'SELECT p.post_id
                        FROM ' . $db_prefix . '_posts p, ' . $db_prefix . '_topics t' . (($sort_order_sql[0] == 'u') ? ', ' . $db_prefix . '_users u' : '') . "
                    ...
                    if (sizeof($post_ids))
                    {
                        $sql = 'SELECT t.topic_id, t.topic_title, t.forum_id, p.post_id, p.post_subject, p.post_username, p.poster_id, p.post_time, u.username, u.clean_username
                ...
                foreach ($rowset as $row)
                {
                    ...
                    $template->assign_block_vars('postrow', array(
                        ...
                        'U_VIEWPOST'        => append_sid("{$phpbb_root_path}forum.$phpEx?action=viewtopic", 'f=' . $row['forum_id'] . '&amp;p=' . $row['post_id']) . (($mode == 'unapproved_posts') ? '#p' . $row['post_id'] : ''),
                        'U_VIEW_DETAILS'    => append_sid("{$phpbb_root_path}forum.$phpEx?action_mcp=mcp", "i=queue&amp;start=$start&amp;mode=approve_details&amp;f={$row['forum_id']}&amp;p={$row['post_id']}" . (($mode == 'unapproved_topics') ? "&amp;t={$row['topic_id']}" : '')),
...
{
    global $db, $db_prefix, $db_prefix, $template, $user, $config, $sitename, $siteurl;
    global $phpEx, $phpbb_root_path, $gfx_check;

    if (!check_ids($post_id_list, $db_prefix . '_posts', 'post_id', array('m_approve')))
    {
    ...
    if (confirm_box(true))
    {
        ...
        if ($notify_poster)
        {
            foreach ($post_info as $post_id => $post_data)
            {
                ...
                $messenger->assign_vars(array(
                    ...
                    'U_VIEW_TOPIC'  => $siteurl . "/forum.$phpEx?action=viewtopic?f={$post_data['forum_id']}&t={$post_data['topic_id']}&e=0",
                    'U_VIEW_POST'   => $siteurl . "/forum.$phpEx?action=viewtopic?f={$post_data['forum_id']}&t={$post_data['topic_id']}&p=$post_id&e=$post_id")
                );
        ...
        if (sizeof($post_id_list) == 1)
        {
            ...
            $post_url = append_sid("{$phpbb_root_path}forum.$phpEx?action=viewtopic", "f={$post_data['forum_id']}&amp;t={$post_data['topic_id']}&amp;p={$post_data['post_id']}") . '#p' . $post_data['post_id'];
        }
        unset($post_info);
...
function disapprove_post($post_id_list, $id, $mode)
{
                ...
                if (isset($disapprove_reason_lang))
                {
                    ...
                    if (!isset($lang_reasons[$post_data['user_lang']]))
                    {
                        ...
                        if ($post_data['user_lang'] != $user->lang_name && file_exists($phpbb_root_path . '/language/' . $post_data['user_lang'] . '/mcp.' . $phpEx))
                        {
                            ...
                            $lang = array();
                            @include($phpbb_root_path . '/language/mcp/' . $post_data['user_lang'] . '.' . $phpEx);
    ...
    $redirect = request_var('redirect', "forum.$phpEx");
    $redirect = $redirect;
