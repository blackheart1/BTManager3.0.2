...
** File mcp.php 2018-09-22 00:00:00 Thor
**
** CHANGES
**
...
include_once($phpbb_root_path . 'include/utf/utf_tools.' . $phpEx);
...
if (isset($_REQUEST['mode']) && is_array($_REQUEST['mode']))
{
    ...
    //list($mode, ) = each($mode);
            foreach($mode as $var=>$val){$mode = $var;}
}
else
{
...
{
            foreach($action_ary as $var=>$val){$action = $var;}
...
$queue_loaded = $reports_loaded = $logs_loaded = true;
...
if (!$auth->acl_getf_global('m_'))
{
    ...
    $allow_user = false;
    if ($quickmod && isset($user_quickmod_actions[$action]) && $user->user && $auth->acl_gets($user_quickmod_actions[$action], $forum_id))
    {
        $topic_info = get_topic_data(array($topic_id));
        if ($topic_info[$topic_id]['topic_poster'] == $user->id)
        {
            $allow_user = true;
        }
    }
...
$module->load_active(false,append_sid("{$phpbb_root_path}forum.$phpEx?action_mcp=mcp"));

// Assign data to the template engine for the list of modules
$module->assign_tpl_vars(append_sid("{$phpbb_root_path}forum.$phpEx?action_mcp=mcp"));

// Generate urls for letting the moderation control panel being accessed in different modes
$template->assign_vars(array(
    'U_MCP'         => append_sid("{$phpbb_root_path}forum.$phpEx?action_mcp=mcp", 'i=main'),
    'U_MCP_FORUM'   => ($forum_id) ? append_sid("{$phpbb_root_path}forum.$phpEx?action_mcp=mcp", "i=main&amp;mode=forum_view&amp;f=$forum_id") : '',
    'U_MCP_TOPIC'   => ($forum_id && $topic_id) ? append_sid("{$phpbb_root_path}forum.$phpEx?action_mcp=mcp", "i=main&amp;mode=topic_view&amp;t=$topic_id") : '',
    'U_MCP_POST'    => ($forum_id && $topic_id && $post_id) ? append_sid("{$phpbb_root_path}forum.$phpEx?action_mcp=mcp", "i=main&amp;mode=post_details&amp;t=$topic_id&amp;p=$post_id") : '',
));
...
{
    global $auth, $db, $config, $user, $db_prefix;
    static $rowset = array();
...
{
    global $db, $auth, $config, $user, $db_prefix;
...
{
    global $auth, $db, $user, $config , $db_prefix;
    ...
    if ($read_tracking && $config['load_db_lastread'])
    {
        $read_tracking_join = ' LEFT JOIN ' . $db_prefix . '_forums_track ft ON (ft.user_id = ' . $user->id . '
    ...
    $sql = "SELECT f.* $read_tracking_select
        FROM " . $db_prefix . "_forums f$read_tracking_join
...
function mcp_sorting($mode, &$sort_days, &$sort_key, &$sort_dir, &$sort_by_sql, &$sort_order_sql, &$total, $forum_id = 0, $topic_id = 0, $where_sql = 'WHERE')
{
    ...
    switch ($type)
    {
        ...
        case 'logs':
            ...
            $sort_by_sql = array('u' => 'u.clen_username', 't' => 'l.datetime', 'i' => 'l.ip', 'o' => 'l.action');
            $limit_time_sql = ($min_time) ? "AND l.datetime >= $min_time" : '';
        break;
    }
