...
** File include/bbcode.php 2018-09-22 00:00:00 Thor
**
** CHANGES
**
...
class bbcode
{
    ...
    */
    ...
    {
        global $siteurl, $template, $user, $theme;

        if (empty($this->template_filename))
        {
            $this->template_bitfield = new bitfield('lNg=');
            $this->template_filename = './themes/' . $theme . '/templates/bbcode.html';
        ...
        foreach ($bbcode_ids as $bbcode_id)
        {
            switch ($bbcode_id)
            {
                ...
                case 3:
                    $this->bbcode_cache[$bbcode_id] = array(
                        'preg' => array(
                            '#\[url:$uid\]((' . $siteurl . '.+?))\[/url:$uid\]#s'           => $this->bbcode_tpl('url', $bbcode_id),
                            '#\[url=(' . $siteurl . '.+?):$uid\](.*?)\[/url:$uid\]#i'   => $this->bbcode_tpl('url', $bbcode_id),
                            '#\[url:$uid\]((.*?))\[/url:$uid\]#s'           => $this->bbcode_tpl('url_ext', $bbcode_id),
                            '#\[url=([^\[]+?):$uid\](.*?)\[/url:$uid\]#s'   => $this->bbcode_tpl('url_ext', $bbcode_id),
                        )
                    );
                break;
                ...
                case 4:
                    if ($user->optionget('viewimg'))
                    {
                        $this->bbcode_cache[$bbcode_id] = array(
                            'preg' => array(
                                '#\[img:$uid\](.*?)\[/img:$uid\]#s'     => $this->bbcode_tpl('img', $bbcode_id,true),
                            )
                        );
                    }
                    else
