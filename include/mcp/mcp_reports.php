...
** File mcp/mcp_reports.php 2018-09-22 00:00:00 Thor
**
** CHANGES
**
...
class mcp_reports
{
    ...
    {
        global $auth, $db_prefix, $db, $user, $template, $pmbt_cache;
        ...
        switch ($mode)
        {
            case 'report_details':
                ...
                $sql = 'SELECT r.post_id, r.user_id, r.report_id, r.report_closed, report_time, r.report_text, rr.reason_title, rr.reason_description, u.username, u.clean_username AS username_clean, u.can_do , l.group_colour AS user_colour
                    FROM ' . $db_prefix . '_reports r, ' . $db_prefix . '_reports_reasons rr, ' . $db_prefix . '_users u, ' . $db_prefix . '_level_settings l
                    WHERE ' . (($report_id) ? 'r.report_id = ' . $report_id : "r.post_id = $post_id") . '
                        AND rr.reason_id = r.reason_id
                        AND r.user_id = u.id
                        AND l.group_id = u.can_do
                    ORDER BY report_closed ASC LIMIT 1';
                    //die($sql);
                ...
                $template->assign_vars(array(
                    'S_MCP_REPORT'          => true,
                    'S_CLOSE_ACTION'        => append_sid("{$phpbb_root_path}forum.$phpEx?action_mcp=mcp", 'i=reports&amp;mode=report_details&amp;f=' . $post_info['forum_id'] . '&amp;p=' . $post_id),
                    ...
                    'U_MCP_REPORTER_NOTES'      => append_sid("{$phpbb_root_path}forum.$phpEx?action_mcp=mcp", 'i=notes&amp;mode=user_notes&amp;u=' . $report['user_id']),
                    'U_MCP_USER_NOTES'          => append_sid("{$phpbb_root_path}forum.$phpEx?action_mcp=mcp", 'i=notes&amp;mode=user_notes&amp;u=' . $post_info['user_id']),
                    'U_MCP_WARN_REPORTER'       => ($auth->acl_get('m_warn')) ? append_sid("{$phpbb_root_path}forum.$phpEx?action_mcp=mcp", 'i=warn&amp;mode=warn_user&amp;u=' . $report['user_id']) : '',
                    'U_MCP_WARN_USER'           => ($auth->acl_get('m_warn')) ? append_sid("{$phpbb_root_path}forum.$phpEx?action_mcp=mcp", 'i=warn&amp;mode=warn_user&amp;u=' . $post_info['user_id']) : '',
                    'U_VIEW_FORUM'              => append_sid("{$phpbb_root_path}forum.$phpEx?action=viewforum", 'f=' . $post_info['forum_id']),
                    ...
                    'RETURN_REPORTS'            => sprintf($user->lang['RETURN_REPORTS'], '<a href="' . append_sid("{$phpbb_root_path}forum.$phpEx?action_mcp=mcp", 'i=reports' . (($post_info['post_reported']) ? '&amp;mode=reports' : '&amp;mode=reports_closed') . '&amp;start=' . $start . '&amp;f=' . $post_info['forum_id']) . '">', '</a>'),
                    ...
                    'U_LOOKUP_IP'           => ($auth->acl_get('m_info', $post_info['forum_id'])) ? "{$phpbb_root_path}forum.$phpEx?action_mcp=mcp" . '&amp;r=' . $report_id . '&amp;p=' . $post_id . '&amp;f=' . $forum_id . '&amp;lookup=' . $post_info['poster_ip'] . '#ip' : '',
                ));
            ...
            case 'reports_closed':
                ...
                $sql = 'SELECT r.report_id
                    FROM ' . $db_prefix . '_posts p, ' . $db_prefix . '_topics t, ' . $db_prefix . '_reports r ' . (($sort_order_sql[0] == 'u') ? ', ' . $db_prefix . '_users u' : '') . (($sort_order_sql[0] == 'r') ? ', ' . $db_prefix . '_users ru' : '') . '
                    WHERE ' . $db->sql_in_set('p.forum_id', $forum_list) . "
                        $report_state
                        AND r.post_id = p.post_id
                        " . (($sort_order_sql[0] == 'u') ? 'AND u.id = p.poster_id' : '') . '
                        ' . (($sort_order_sql[0] == 'r') ? 'AND ru.id = p.poster_id' : '') . '
                ...
                {
                    $sql = 'SELECT t.forum_id, t.topic_id, t.topic_title, p.post_id, p.post_subject, p.post_username, p.poster_id, p.post_time, u.username, u.clean_username AS username_clean, u.can_do , l.group_colour AS user_colour, r.user_id as reporter_id, ru.username as reporter_name, ru.can_do , lu.group_colour AS reporter_colour, r.report_time, r.report_id
                        FROM ' . $db_prefix . '_reports r, ' . $db_prefix . '_posts p, ' . $db_prefix . '_topics t, ' . $db_prefix . '_users u, ' . $db_prefix . '_users ru, ' . $db_prefix . '_level_settings l, ' . $db_prefix . '_level_settings lu
                    ...
                    while ($row = $db->sql_fetchrow($result))
                    {
                        ...
                        $template->assign_block_vars('postrow', array(
                            'U_VIEWFORUM'               => (!$global_topic) ? append_sid("{$phpbb_root_path}forum.$phpEx?action=viewforum", 'f=' . $row['forum_id']) : '',
                            'U_VIEWPOST'                => append_sid("{$phpbb_root_path}forum.$phpEx?action=viewtopic", 'f=' . $row['forum_id'] . '&amp;p=' . $row['post_id']) . '#p' . $row['post_id'],
                            'U_VIEW_DETAILS'            => append_sid("{$phpbb_root_path}forum.$phpEx?action_mcp=mcp", "i=reports&amp;start=$start&amp;mode=report_details&amp;f={$row['forum_id']}&amp;r={$row['report_id']}"),
...
{
    global $db, $db_prefix, $template, $user, $config;
    global $phpEx, $phpbb_root_path;

    $sql = 'SELECT r.post_id
        FROM ' . $db_prefix . '_reports r
    ...
    if (!check_ids($post_id_list, $db_prefix . '_posts', 'post_id', array('m_report')))
    {
    ...
    if (confirm_box(true))
    {
        ...
        $sql = 'SELECT r.report_id, r.post_id, r.report_closed, r.user_id, r.user_notify, u.username, u.clean_username AS username_clean, u.email AS user_email, u.jabber AS user_jabber, u.language AS user_lang, u.user_notify_type
            FROM ' . $db_prefix . '_reports r, ' . $db_prefix . '_users u
            WHERE ' . $db->sql_in_set('r.report_id', $report_id_list) . '
        ...
        if (sizeof($reports))
        {
            ...
            if ($action == 'close')
            {
                $sql = 'UPDATE ' . $db_prefix . '_reports
                    SET report_closed = 1
                    WHERE ' . $db->sql_in_set('report_id', $report_id_list);
            }
            else
            {
                $sql = 'DELETE FROM ' . $db_prefix . '_reports
                    WHERE ' . $db->sql_in_set('report_id', $report_id_list);
            }
        ...
        if (sizeof($notify_reporters))
        {
            foreach ($notify_reporters as $report_id => $reporter)
            {
                ...
                $messenger->assign_vars(array(
                    ...
                    'CLOSER_NAME'   => htmlspecialchars_decode($user->name),
    ...
    else
    {
        ...
        {
            $return_forum = sprintf($user->lang['RETURN_FORUM'], '<a href="' . append_sid("{$phpbb_root_path}forum.$phpEx?action=viewforum", 'f=' . current($forum_ids)) . '">', '</a>') . '<br /><br />';
        }
        $return_topic = '';
