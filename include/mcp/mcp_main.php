...
** File mcp/mcp_main.php 2018-09-22 00:00:00 Thor
**
** CHANGES
**
...
class mcp_main
{
    ...
    function main($id, $mode)
    {
        ...
        switch ($action)
        {
            ...
            case 'move':
                //$user->add_lang('viewtopic',$user->ulanguage);
            ...
            case 'delete_post':
                //$user->add_lang('posting',$user->ulanguage);
        ...
        {
            case 'front':
                include_once($phpbb_root_path . 'include/mcp/mcp_front.' . $phpEx);
            ...
            case 'forum_view':
                include_once($phpbb_root_path . 'include/mcp/mcp_forum.' . $phpEx);
            ...
            case 'topic_view':
                include_once($phpbb_root_path . 'include/mcp/mcp_topic.' . $phpEx);
            ...
            case 'post_details':
                include_once($phpbb_root_path . 'include/mcp/mcp_post.' . $phpEx);
...
{
    global $auth, $user, $db, $db_prefix, $phpEx, $phpbb_root_path;

    if ($action == 'lock' || $action == 'unlock')
    {
        $table = $db_prefix . '_topics';
        $sql_id = 'topic_id';
        $set_id = 'topic_status';
        $l_prefix = 'TOPIC';
    }
    ...
    {
        $sql = "UPDATE $table
            SET $set_id = " . (($action == 'lock' || $action == 'lock_post') ? 1 : 0) . '
...
function change_topic_type($action, $topic_ids)
{
    ...
    $forum_id = check_ids($topic_ids, $db_prefix . '_topics', 'topic_id', array('f_announce', 'f_sticky', 'm_'), true);
...
{
    global $auth, $user, $db, $db_prefix, $template;
    global $phpEx, $phpbb_root_path;

    // Here we limit the operation to one forum only
    $forum_id = check_ids($topic_ids, $db_prefix . '_topics', 'topic_id', array('m_move'), true);
    ...
    if (confirm_box(true))
    {
        ...
        foreach ($topic_data as $topic_id => $row)
        {
            ...
            if ($leave_shadow && $row['topic_approved'] && $row['topic_type'] != 3)
            {
                $shadow = array(
                ...
                $db->sql_query('INSERT INTO ' . $db_prefix . '_topics' . $db->sql_build_array('INSERT', $shadow));
    ...
    {
        $template->assign_vars(array(
            'S_FORUM_SELECT'        => make_forum_select($to_forum_id, $forum_id, true, true, true, true),
...
function mcp_delete_topic($topic_ids)
{
    ...
    if (!check_ids($topic_ids, $db_prefix . '_topics', 'topic_id', array('m_delete')))
    {
        return;
    }
    ...
    {
        $redirect = append_sid("{$phpbb_root_path}forum.$phpEx?action=viewforum", 'f=' . $forum_id);
...
function mcp_delete_post($post_ids)
{
    ...
    if (!check_ids($post_ids, $db_prefix . '_posts', 'post_id', array('m_delete')))
    {
        return;
    }
    ...
    if (confirm_box(true))
    {
        ...
        {
            $return_link[] = sprintf($user->lang['RETURN_TOPIC'], '<a href="' . append_sid("{$phpbb_root_path}forum.$phpEx?action=viewtopic", "f=$forum_id&amp;t=$topic_id") . '">', '</a>');
        }
        $return_link[] = sprintf($user->lang['RETURN_FORUM'], '<a href="' . append_sid("{$phpbb_root_path}forum.$phpEx?action=viewforum", 'f=' . $forum_id) . '">', '</a>');
    ...
    else
    {
        if ($affected_topics != 1 || $deleted_topics || !$topic_id)
        {
            $redirect = append_sid("{$phpbb_root_path}forum.$phpEx?action_mcp=mcp", "f=$forum_id&i=main&mode=forum_view", false);
        }
...
{
    global $auth, $user, $db, $db_prefix, $template, $config;
    global $phpEx, $phpbb_root_path;

    if (!check_ids($topic_ids, $db_prefix . '_topics', 'topic_id', array('m_')))
    {
        return;
    }
    ...
    if (confirm_box(true))
    {
        ...
        foreach ($topic_data as $topic_id => $topic_row)
        {
            ...
            $db->sql_query('INSERT INTO ' . $db_prefix . '_topics ' . $db->sql_build_array('INSERT', $sql_ary));
            ...
            if ($topic_row['poll_start'])
            {
                ...
                while ($row = $db->sql_fetchrow($result))
                {
                    ...
                    $db->sql_query('INSERT INTO ' . $db_prefix . '_poll_options ' . $db->sql_build_array('INSERT', $sql_ary));
                }
            }
            ...
            foreach ($post_rows as $row)
            {
                ...
                $db->sql_query('INSERT INTO ' . $db_prefix . '_posts ' . $db->sql_build_array('INSERT', $sql_ary));
                ...
                {
                    $sql = 'SELECT * FROM ' . $db_prefix . "_attachments
                    ...
                    if (sizeof($sql_ary))
                    {
                        $db->sql_multi_insert($db_prefix . '_attachments', $sql_ary);
                    }
                }
            }
            ...
            if (sizeof($sql_ary))
            {
                $db->sql_multi_insert($db_prefix . '_topics_watch', $sql_ary);
            }
        }
    ...
    else
    {
        ...
        {
            $return_link .= '<br /><br />' . sprintf($user->lang['RETURN_NEW_FORUM'], '<a href="' . append_sid("{$phpbb_root_path}forum.$phpEx?action=viewforum", 'f=' . $to_forum_id) . '">', '</a>');
        }
