...
** File include/modules.php 2018-09-22 00:00:00 Thor
**
** CHANGES
**
...
class p_master
{
    ...
    */
    function __construct($include_path = false)
    {
        ...
        $this->include_path = ($include_path !== false) ? $include_path : $phpbb_root_path . 'include/';
    ...
    */
    ...
    {
        global $auth, $db, $db_prefix, $user, $pmbt_cache;
        ...
        // Get cached modules
        if (($this->module_cache = $pmbt_cache->get('_modules_' . $this->p_class)) === false)
        {
            // Get modules
            $sql = 'SELECT *
            ...
            $pmbt_cache->put('_modules_' . $this->p_class, $this->module_cache);
        }
    ...
    */
    function load_active($mode = false, $module_url = false, $execute_module = true)
    {
        ...
        {
            trigger_error('Module not accessible' . $this->include_path . $this->p_class, E_USER_ERROR);
        }
        ...
        if (!class_exists("{$this->p_class}_$this->p_name"))
        {
            ...
            if (defined('IN_ADMIN'))
            {
                ...
                $this->module->u_action = append_sid("{$phpbb_admin_path}admin.$phpEx", "i={$this->p_name}") . (($icat) ? '&amp;icat=' . $icat : '') . "&amp;mode={$this->p_mode}";
            }
            else
            {
    ...
    */
    function display($page_title, $display_online_list = true)
    {
        ...
        echo $template->fetch($this->get_tpl_name());
        close_out();
    }
