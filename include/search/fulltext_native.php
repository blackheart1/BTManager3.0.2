...
** File search/fulltext_native.php 2018-09-22 00:00:00 Thor
**
** CHANGES
**
...
* @ignore
*/
include_once('include/search/search.php');
    ...
    */
    function __construct(&$error)
    {
        ...
        */
        ...
        {
            include($phpbb_root_path . 'include/utf/utf_normalizer.' . $phpEx);
        }
    ...
    function split_keywords($keywords, $terms)
    {
        ...
        if (sizeof($exact_words))
        {
            $sql = 'SELECT word_id, word_text, word_common
                FROM ' . $db_prefix . '_search_wordlist
    ...
    */
    function keyword_search($type, &$fields, &$terms, &$sort_by_sql, &$sort_key, &$sort_dir, &$sort_days, &$ex_fid_ary, &$m_approve_fid_ary, &$topic_id, &$author_ary, &$id_ary, $start, $per_page)
    {
        ...
        $sql_array = array(
            ...
            'FROM'      => array(
                $db_prefix . '_search_wordmatch'    => array(),
                $db_prefix . '_search_wordlist' => array(),
            ),
            'LEFT_JOIN' => array(array(
                'FROM'  => array($db_prefix . '_posts' => 'p'),
        ...
        */
        ...
        foreach ($this->must_contain_ids as $subquery)
        {
                ...
                foreach ($subquery as $id)
                {
                    if (is_string($id))
                    {
                        $sql_array['LEFT_JOIN'][] = array(
                            'FROM'  => array($db_prefix . '_search_wordlist' => 'w' . $w_num),
            ...
            {
                $sql_array['FROM'][$db_prefix . '_search_wordlist'][] = 'w' . $w_num;
            ...
            $sql_array['FROM'][$db_prefix . '_search_wordmatch'][] = 'm' . $m_num;
        ...
        {
            $sql_array['LEFT_JOIN'][] = array(
                'FROM'  => array($db_prefix . '_search_wordmatch' => 'm' . $m_num),
        ...
        {
            case 'u':
                $sql_array['FROM'][$db_prefix . '_users'] = 'u';
                $sql_where[] = 'u.id = p.poster_id ';
            break;
            ...
            case 'f':
                $sql_array['FROM'][$db_prefix . '_forums'] = 'f';
    ...
    function author_search($type, $firstpost_only, &$sort_by_sql, &$sort_key, &$sort_dir, &$sort_days, &$ex_fid_ary, &$m_approve_fid_ary, &$topic_id, &$author_ary, &$id_ary, $start, $per_page)
    {
        ...
        switch ($sql_sort[0])
        {
            case 'u':
                ...
                $sql_sort_join  = ' AND u.id = p.poster_id ';
            break;

            case 't':
                $sql_sort_table = ($type == 'posts' && !$firstpost_only) ? $db_prefix . '_posts t, ' : '';
...
//                  $select = 'SQL_CALC_FOUND_ROWS ' . $select;
                ...
                default:
                    if ($type == 'posts')
                    {
                        $sql = 'SELECT COUNT(p.post_id) as total_results
                            FROM ' . $db_prefix . '_posts p' . (($firstpost_only) ? ', ' . $db_prefix . '_topics t ' : ' ') . "
                            WHERE $sql_author
                                $sql_topic_id
                                $sql_firstpost
                                $m_approve_fid_sql
                    ...
                    else
                    {
                        ...
                        $sql .= ' FROM ' . $db_prefix . '_topics t, ' . $db_prefix . "_posts p
                            WHERE $sql_author
                                $sql_topic_id
                                $sql_firstpost
                                $m_approve_fid_sql
        ...
        //die($sql . ' LIMIT ' . 50 .  ', ' . $start);
        $result = $db->sql_query($sql . ' LIMIT ' . $start .  ', ' . 50) or die(mysql_error());
    ...
    */
    ...
    {
        global $config, $db, $user, $db_prefix;
        ...
        if ($mode == 'edit')
        {
            ...
            $sql = 'SELECT w.word_id, w.word_text, m.title_match
                FROM ' . $db_prefix . '_search_wordlist w, ' . $db_prefix . "_search_wordmatch m
        ...
        foreach ($words['del'] as $word_in => $word_ary)
        {
            ...
            if (sizeof($word_ary))
            {
                ...
                $sql = 'DELETE FROM ' . $db_prefix . '_search_wordmatch
                    WHERE ' . $db->sql_in_set('word_id', $sql_in) . '
                        AND post_id = ' . intval($post_id) . "
                        AND title_match = $title_match";
                $db->sql_query($sql);

                $sql = 'UPDATE ' . $db_prefix . '_search_wordlist
        ...
        foreach ($words['add'] as $word_in => $word_ary)
        {
            ...
            if (sizeof($word_ary))
            {
                $sql = 'INSERT INTO ' . $db_prefix . '_search_wordmatch (post_id, word_id, title_match)
    ...
    */
    function index_remove($post_ids, $author_ids, $forum_ids)
    {
        ...
        {
            $sql = 'SELECT w.word_id, w.word_text, m.title_match
                FROM ' . $db_prefix . '_search_wordmatch m, ' . $db_prefix . '_search_wordlist w
    ...
    */
    function delete_index($acp_module, $u_action)
    {
        ...
        switch ($db->sql_layer)
        {
            case 'sqlite':
            case 'firebird':
                $db->sql_query('DELETE FROM ' . $db_prefix . '_search_wordlist');
                $db->sql_query('DELETE FROM ' . $db_prefix . '_search_wordmatch');
                $db->sql_query('DELETE FROM ' . $db_prefix . '_search_results');
            break;

            default:
                $db->sql_query('TRUNCATE TABLE ' . $db_prefix . '_search_wordlist');
                $db->sql_query('TRUNCATE TABLE ' . $db_prefix . '_search_wordmatch');
    ...
    */
    ...
    function get_stats()
    {
        ...
        $sql = 'SELECT COUNT(*) as total_matches
            FROM ' . $db_prefix . '_search_wordmatch';
        $result = $db->sql_query($sql);
        ...
        */
        ...
        do
        {
            ...
            */
            ...
            {
                $conv_loaded[$idx] = 1;
                $file = $phpbb_root_path . 'include/utf/data/search_indexer_' . $idx . '.' . $phpEx;
