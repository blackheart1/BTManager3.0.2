...
** File mcp/mcp_topic.php 2018-09-22 00:00:00 Thor
**
** CHANGES
**
...
function mcp_topic_view($id, $mode, $action)
{
    ...
    global $template, $db, $user, $auth, $db_prefix, $pmbt_cache;
    ...
    {
        include($phpbb_root_path . 'include/mcp/mcp_queue.' . $phpEx);
    ...
    $sql = 'SELECT u.username, u.clean_username, p.*, u.can_do , l.group_colour AS user_colour
    ...
    {
        $extensions = $pmbt_cache->obtain_attach_extensions($topic_info['forum_id']);
    ...
    //die(print_r($row));
        ...
        $template->assign_block_vars('postrow', array(
            ...
            'MESSAGE'       => $message,
            'POST_ID'       => $row['post_id'],
            'RETURN_TOPIC'  => sprintf($user->lang['RETURN_TOPIC'], '<a href="' . append_sid("{$phpbb_root_path}forum.$phpEx?action=viewtopic", 't=' . $topic_id) . '">', '</a>'),
        'INFO_IMG'          => $user->img('icon_post_info', 'VIEW_INFO'),
            ...
            'U_MCP_APPROVE'     => ($auth->acl_get('m_approve', $topic_info['forum_id'])) ? append_sid("{$phpbb_root_path}forum.$phpEx?action_mcp=mcp", 'i=queue&amp;mode=approve_details&amp;f=' . $topic_info['forum_id'] . '&amp;p=' . $row['post_id']) : '',
            'U_MCP_REPORT'      => ($auth->acl_get('m_report', $topic_info['forum_id'])) ? append_sid("{$phpbb_root_path}forum.$phpEx?action_mcp=mcp", 'i=reports&amp;mode=report_details&amp;f=' . $topic_info['forum_id'] . '&amp;p=' . $row['post_id']) : '')
        );
    ...
    $template->assign_vars(array(
        ...
        'U_VIEW_TOPIC'      => append_sid("{$phpbb_root_path}forum.$phpEx?action=viewtopic", 'f=' . $topic_info['forum_id'] . '&amp;t=' . $topic_info['topic_id']),

        'TO_TOPIC_ID'       => $to_topic_id,
        'TO_TOPIC_INFO'     => ($to_topic_id) ? sprintf($user->lang['YOU_SELECTED_TOPIC'], $to_topic_id, '<a href="' . append_sid("{$phpbb_root_path}forum.$phpEx?action=viewtopic", 'f=' . $to_topic_info['forum_id'] . '&amp;t=' . $to_topic_id) . '">' . $to_topic_info['topic_title'] . '</a>') : '',
        ...
        'RETURN_TOPIC'      => sprintf($user->lang['RETURN_TOPIC'], '<a href="' . append_sid("{$phpbb_root_path}forum.$phpEx?action=viewtopic", "f={$topic_info['forum_id']}&amp;t={$topic_info['topic_id']}&amp;start=$start") . '">', '</a>'),
        'RETURN_FORUM'      => sprintf($user->lang['RETURN_FORUM'], '<a href="' . append_sid("{$phpbb_root_path}forum.$phpEx?action=viewforum", "f={$topic_info['forum_id']}&amp;start=$start") . '">', '</a>'),

        'PAGE_NUMBER'       => on_page($total, $posts_per_page, $start),
        'PAGINATION'        => (!$posts_per_page) ? '' : generate_pagination(append_sid("{$phpbb_root_path}forum.$phpEx?action_mcp=mcp", "i=$id&amp;t={$topic_info['topic_id']}&amp;mode=$mode&amp;action=$action&amp;to_topic_id=$to_topic_id&amp;posts_per_page=$posts_per_page&amp;st=$sort_days&amp;sk=$sort_key&amp;sd=$sort_dir"), $total, $posts_per_page, $start),
...
{
    global $db, $db_prefix, $template, $user, $phpEx, $phpbb_root_path, $auth, $config;
    ...
    if (!check_ids($post_id_list, $db_prefix . '_posts', 'post_id', array('m_split')))
    {
        return;
    }
        ...
        if ($action == 'split_beyond')
        {
            ...
            if ($sort_order_sql[0] == 'u')
            {
                $sql = 'SELECT p.post_id, p.forum_id, p.post_approved
                    FROM ' . $db_prefix . '_posts p, ' . $db_prefix . "_users u
...
//                  continue;
        ...
        $sql = 'INSERT INTO ' . $db_prefix . '_topics ' . $db->sql_build_array('INSERT', $sql_ary);
        $db->sql_query($sql);
        ...
        $return_link = sprintf($user->lang['RETURN_TOPIC'], '<a href="' . append_sid("{$phpbb_root_path}forum.$phpEx?action=viewtopic", 'f=' . $post_info['forum_id'] . '&amp;t=' . $post_info['topic_id']) . '">', '</a>') . '<br /><br />' . sprintf($user->lang['RETURN_NEW_TOPIC'], '<a href="' . append_sid("{$phpbb_root_path}forum.$phpEx?action=viewtopic", 'f=' . $to_forum_id . '&amp;t=' . $to_topic_id) . '">', '</a>');
    }
    else
    {
    ...
    {
        meta_refresh(3, append_sid("{$phpbb_root_path}forum.$phpEx?action=viewtopic", "f=$to_forum_id&amp;t=$to_topic_id"));
...
{
    global $db, $db_prefix, $template, $user, $phpEx, $phpbb_root_path, $auth;
    ...
    if (!check_ids($post_id_list, $db_prefix . '_posts', 'post_id', array('m_merge')))
    {
        return;
    }
    ...
    if (confirm_box(true))
    {
        ...
        if ($row)
        {
            $return_link .= sprintf($user->lang['RETURN_TOPIC'], '<a href="' . append_sid("{$phpbb_root_path}forum.$phpEx?action=viewtopic", 'f=' . $row['forum_id'] . '&amp;t=' . $topic_id) . '">', '</a>');
        }
        else
        {
            ...
            $db->sql_query('UPDATE ' . $db_prefix . '_topics_watch SET topic_id = ' . (int) $to_topic_id . ' WHERE topic_id = ' . (int) $topic_id);
            $db->sql_return_on_error(false);

            $db->sql_query('DELETE FROM ' . $db_prefix . '_topics_watch WHERE topic_id = ' . (int) $topic_id);
        }

        // Link to the new topic
        $return_link .= (($return_link) ? '<br /><br />' : '') . sprintf($user->lang['RETURN_NEW_TOPIC'], '<a href="' . append_sid("{$phpbb_root_path}forum.$phpEx?action=viewtopic", 'f=' . $to_forum_id . '&amp;t=' . $to_topic_id) . '">', '</a>');
    }
    else
    {
