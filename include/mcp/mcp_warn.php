...
** File mcp/mcp_warn.php 2018-09-22 00:00:00 Thor
**
** CHANGES
**
...
class mcp_warn
{
    ...
    */
    function mcp_warn_front_view()
    {
        ...
        $template->assign_vars(array(
            ...
            'U_POST_ACTION'     => append_sid("{$phpbb_root_path}forum.$phpEx?action_mcp=mcp", 'i=warn&amp;mode=warn_user'),
        ));
        ...
        if(!is_array($highest))$highest = array();
        ...
        $sql = 'SELECT u.id AS user_id, u.username, u.clean_username AS username_clean, u.can_do , l.group_colour AS user_colour, u.user_warnings, w.warning_time
            FROM ' . $db_prefix . '_users u, ' . $db_prefix . '_warnings w, ' . $db_prefix . '_level_settings l
    ...
    */
    function mcp_warn_list_view($action)
    {
        ...
        $sort_by_sql = array('a' => 'clean_username', 'b' => 'user_last_warning', 'c' => 'user_warnings');
        ...
        $template->assign_vars(array(
            ...
            'PAGINATION'        => generate_pagination(append_sid("{$phpbb_root_path}forum.$phpEx?action_mcp=mcp", "i=warn&amp;mode=list&amp;st=$st&amp;sk=$sk&amp;sd=$sd"), $user_count, $config['topics_per_page'], $start),
    ...
    */
    function mcp_warn_post_view($action)
    {
        ...
        $template->assign_vars(array(
            ...
            'JOINED'            => $user->format_date($user_row['regdate']),
    ...
    */
    function mcp_warn_user_view($action)
    {
        ...
        $template->assign_vars(array(
            ...
            'USERNAME_FULL'     => get_username_string('full', $user_row['id'], $user_row['username'], $user_row['user_colour']),
            'USERNAME_COLOUR'   => get_username_string('colour', $user_row['id'], $user_row['username'], $user_row['user_colour']),
            'USERNAME'          => get_username_string('username', $user_row['id'], $user_row['username'], $user_row['user_colour']),
            'U_PROFILE'         => get_username_string('profile', $user_row['id'], $user_row['username'], $user_row['user_colour']),
...
function add_warning($user_row, $warning, $send_pm = true, $post_id = 0)
{
    ...
    if ($send_pm)
    {
        include_once($phpbb_root_path . 'include/ucp/functions_privmsgs.' . $phpEx);
        include_once($phpbb_root_path . 'include/message_parser.' . $phpEx);

        $user_row['user_lang'] = (file_exists($phpbb_root_path . 'language/mcp/' . $user_row['language'] . ".php")) ? $user_row['language'] : $user->ulanguage;
        include($phpbb_root_path . 'language/mcp/' . basename($user_row['user_lang']) . ".$phpEx");
        ...
        $pm_data = array(
            ...
            'address_list'          => array('u' => array($user_row['id'] => 'to')),
        );
    ...
    $log_id = add_log('user', $user_row['id'], 'LOG_USER_WARNING_BODY', $warning);
    ...
    $db->sql_query('INSERT INTO ' . $db_prefix . '_warnings ' . $db->sql_build_array('INSERT', $sql_ary))or btsqlerror('INSERT INTO ' . $db_prefix . '_warnings ' . $db->sql_build_array('INSERT', $sql_ary));
