...
** File acp/auth.php 2018-09-22 00:00:00 Thor
**
** CHANGES
**
...
class auth_admin extends auth
{
    ...
    */
    function get_mask($mode, $user_id = false, $group_id = false, $forum_id = false, $auth_option = false, $scope = false, $acl_fill = 0)
    {
        ...
        if ($view_user_mask)
        {
            ...
            $sql = 'SELECT id, user_permissions, user_type
                FROM ' . $db_prefix . '_users
                WHERE ' . $db->sql_in_set('id', $ug_id);
            $result = $db->sql_query($sql);
            ...
            while ($userdata = $db->sql_fetchrow($result))
            {
                ...
                $hold_ary[$userdata['id']] = array();
                foreach ($forum_ids as $f_id)
                {
                    $hold_ary[$userdata['id']][$f_id] = array();
                    foreach ($compare_options as $option)
                    {
                        $hold_ary[$userdata['id']][$f_id][$option] = $auth2->acl_get($option, $f_id);
                    }
                }
            }
            $db->sql_freeresult($result);
    ...
    */
    ...
    {
        global $template, $user, $db, $db_prefix, $phpbb_root_path, $phpEx;
        ...
        if ($user_mode == 'user')
        {
            $sql = 'SELECT id as ug_id, username as ug_name
                FROM ' . $db_prefix . '_users
        ...
        while ($row = $db->sql_fetchrow($result))
        {
            $ug_names_ary[$row['ug_id']] = ($user_mode == 'user') ? $row['ug_name'] : (($row['group_type'] == 3) ? $user->lang['G_' . $row['ug_name']] : $row['ug_name']);
        }
        $db->sql_freeresult($result);
        ...
        if ($user_mode == 'user' && $group_display)
        {
            ...
            if ($memberships)
            {
                ...
                {
                    if ($groups[$row['group_id']]['group_type'] == 3)
                    {
    ...
    */
    ...
    {
        global $db,$db_prefix, $template, $user, $phpbb_root_path, $phpbb_admin_path, $phpEx;
        ...
        foreach ($forum_names as $forum_id => $forum_name)
        {
            ...
            if (isset($auth_ary['users']) && sizeof($auth_ary['users']))
            {
                $sql = 'SELECT id, username
                    FROM ' . $db_prefix . '_users
                    WHERE ' . $db->sql_in_set('id', $auth_ary['users']) . '
                    ORDER BY clean_username ASC';
                $result = $db->sql_query($sql);
                ...
                while ($row = $db->sql_fetchrow($result))
                {
                    $template->assign_block_vars('role_mask.users', array(
                        ...
                        'U_PROFILE'     => append_sid("{$phpbb_root_path}user.$phpEx", "op=profile&amp;id={$row['id']}"))
                    );
                }
                $db->sql_freeresult($result);
            }
    ...
    */
    function acl_add_option($options)
    {
        ...
        {
            $db->sql_multi_insert($db_prefix . '_acl_options', $add_options);
        }

        if (!empty($update_options))
        {
            $sql = 'UPDATE ' . $db_prefix . '_acl_options
                SET is_global = 1, is_local = 1
                WHERE ' . $db->sql_in_set('auth_option', $update_options);
            $db->sql_query($sql);
        }

        $pmbt_cache->remove_file('_acl_options');
    ...
    */
    function acl_set($ug_type, $forum_id, $ug_id, $auth, $role_id = 0, $clear_prefetch = true)
    {
        ...
        $table = ($ug_type == 'user') ? $db_prefix . '_acl_users' : $db_prefix . '_acl_groups';
        ...
        {
            if ($setting == 1 && (!isset($auth[$flag]) || $auth[$flag] == 0))
            {
                $auth[$flag] = 1;
            }
        }
    ...
    */
    function acl_set_role($role_id, $auth)
    {
        ...
        $db->sql_multi_insert($db_prefix . '_acl_roles_data', $sql_ary);
    ...
    */
    function acl_delete($mode, $ug_id = false, $forum_id = false, $permission_type = false)
    {
        ...
        $option_id_ary = array();
        $table = ($mode == 'user') ? $db_prefix . '_acl_users' : $db_prefix . '_acl_groups';
        $id_field = $mode . '_id';
        ...
        if ($permission_type !== false)
        {
            ...
            $sql = "SELECT auth_role_id, $id_field, forum_id
                FROM $table, " . $db_prefix . "_acl_roles r
            ...
            if (sizeof($cur_role_auth))
            {
                $sql = 'SELECT ao.auth_option, rd.role_id, rd.auth_setting
                    FROM ' . $db_prefix . '_acl_options ao, ' . $db_prefix . '_acl_roles_data rd
            ...
            foreach($cat_array['permissions'] as $permission=>$allowed)
            {
                if ($s_view)
                {
                    $template->assign_block_vars($tpl_cat . '.' . $tpl_mask, array(
                        ...
                        'U_TRACE'       => ($show_trace) ? append_sid("{$phpbb_admin_path}admin.$phpEx", "op=levels&amp;action=permissions&amp;mode=trace&amp;u=$ug_id&amp;f=$forum_id&amp;auth=$permission") : '',
                        'UA_TRACE'      => ($show_trace) ? append_sid("{$phpbb_admin_path}admin.$phpEx", "op=levels&amp;action=permissions&mode=trace&u=$ug_id&f=$forum_id&auth=$permission", false) : '',
    ...
    function build_permission_array(&$permission_row, &$content_array, &$categories, $key_sort_array)
    {
        ...
        foreach ($key_sort_array as $forum_id)
        {
            ...
            foreach ($permissions as $permission=>$auth_setting)
            {
                ...
                $content_array[$forum_id][$cat]['S_YES'] |= ($auth_setting == 1) ? true : false;
                $content_array[$forum_id][$cat]['S_NEVER'] |= ($auth_setting == 0) ? true : false;
                $content_array[$forum_id][$cat]['S_NO'] |= ($auth_setting == -1) ? true : false;
    ...
    */
    function ghost_permissions($from_user_id, $to_user_id)
    {
        ...
        foreach ($this->acl_options['global'] as $opt => $id)
        {
            if (strpos($opt, 'a_') === 0)
            {
                $hold_ary[0][$this->acl_options['id'][$opt]] = 0;
            }
        }

        // Force a_switchperm to be allowed
        $hold_ary[0][$this->acl_options['id']['a_switchperm']] = 1;
