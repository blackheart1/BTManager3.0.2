...
** File include/function_attach.php 2018-09-22 00:00:00 Thor
**
** CHANGES
**
...
class filespec
{
    ...
    */
    function clean_filename($mode = 'unique', $prefix = '', $user_id = '')
    {
        ...
        switch ($mode)
        {
            ...
            case 'unique':
                $this->realname = $prefix . md5(RandomAlpha(9) . microtime());
            break;
            ...
            case 'unique_ext':
            default:
                $this->realname = $prefix . md5(RandomAlpha(9) . microtime()) . '.' . $this->extension;
            break;
        }
    }
    ...
    */
    function move_file($destination, $overwrite = false, $skip_image_check = false, $chmod = 0666)
    {
        ...
        {
        $this->error[] = $this->destination_path;
...
class fileupload
{
    ...
    */
    function remote_upload($upload_url)
    {
        ...
        $tmp_path = (!@ini_get('safe_mode')) ? false : $siteurl . '/cache';
        $filename = tempnam($tmp_path, RandomAlpha(9) . microtime() . '-');
