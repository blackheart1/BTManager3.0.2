...
** File subscribed.php 2018-09-22 00:00:00 Thor
**
** CHANGES
**
...
$user->set_lang('forum',$user->ulanguage);
    ...
    while($dcont = $db->sql_fetchrow($res)){
        ...
        //die(print_r($userrow));
                ...
                'CAN_EDIT'              => ($torrent["owner"] == $user->id AND $auth->acl_get("u_edit_torrents"))? true : ($auth->acl_get("m_can_edit_torrents")? true : false),
                'CAN_DELETE'            => ($torrent["owner"] == $user->id AND $auth->acl_get("u_delete_torrents"))? true : ($auth->acl_get("m_delete_torrents")? true : false),
